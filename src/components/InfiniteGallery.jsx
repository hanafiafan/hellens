...
/* Adapted from the 3D gallery component: JSX (no TS), auto-play only.
 * The original hijacked the global wheel + `document.querySelector('canvas')`,
 * which breaks page scroll and grabs the wrong canvas on a page that already
 * has a WebGL hero — so interaction input is removed; the corridor self-drives. */
  ...
  new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        if (blurAmount > 0.0) {
          vec2 texelSize = vec2(0.0015); // approx; avoids GLSL3 textureSize()
...
function GalleryScene({
  ...
  // Total corridor advance for a full 0->1 scroll of the section (several depth
  // cycles so the stream rushes through the whole photo pool).
  const ADVANCE = depthRange * 4;

  // Resume auto-play after 3s of no interaction (matches the reference).
  useEffect(() => {
  ...
  useFrame((state, delta) => {
    // Scroll-driven when the section provides progress; otherwise gentle auto-play.
    const p = driveRef && typeof driveRef.current === 'number' ? driveRef.current : null;
    const dt = delta || 0.0001;
    if (p != null) {
      if (prevDrive.current == null) prevDrive.current = p;
      const dp = p - prevDrive.current;
      prevDrive.current = p;
      if (Math.abs(dp) > 0.00003) {
        lastInteraction.current = Date.now();
        if (autoPlay) setAutoPlay(false);
        // plane advance = vel*dt*10 ; want it to equal dp*ADVANCE
        setScrollVelocity((dp * ADVANCE) / (dt * 10));
      } else {
        setScrollVelocity((prev) => (autoPlay ? prev + 0.3 * dt * speed : prev) * 0.9);
      }
    } else {
      setScrollVelocity((prev) => (autoPlay ? prev + 0.3 * dt * speed : prev) * 0.95);
    }
